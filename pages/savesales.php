...
if($d=='cash') {
	$f = $_POST['cash'];
	$sql = "INSERT INTO sales (invoice_number,cashier,date,type,amount,cash,name,mobile_no,doctor,month,year,p_amount,vat) VALUES (:a,:b,:c,:d,:e,:f,:g,:m,:n,:h,:i,:k,:j)";
	$q = $db->prepare($sql);
	$q->execute(array(':a'=>$a,':b'=>$b,':c'=>$c,':d'=>$d,':e'=>$e,':f'=>$f,':g'=>$cname,':m'=>$mob,':n'=>$docter,':h'=>$dmonth,':i'=>$dyear,':k'=>$pamount,':j'=>$vat));
	header("location: preview.php?invoice=$a");
	exit();
}
// query
